...
boolean tokenizerAcceptNumber(Tokenizer *self) {
   char c = sstrmPeek(STRM);
...
boolean tokenizerAcceptIdentChar(Tokenizer *self) {
...
boolean tokenizerAcceptGrammar(Tokenizer *self) {
   static char chars[] = { '{', '}', '(', ')', ';' };
   int i;
...
TYPE        ::= "int" | "String" | "float" | "boolean" | "Blob" | "char"
...
boolean tstrmAtEnd(TokenStream *self) {
   return self->pos >= self->last;
}
...
boolean tstrmAcceptIdentifier(TokenStream *self, StringView id) {
...
boolean tstrmAcceptOperator(TokenStream *self, char c) {
   Token t = tstrmPeek(self);
